...
# Add Agent 1 src to path
AGENT1_SRC = Path(__file__).resolve().parent / "Agent_1(browser_use+Playwright)" / "src"
sys.path.insert(0, str(AGENT1_SRC))

# Add Agent 2 to path
AGENT2_DIR = Path(__file__).resolve().parent / "Agent_2(AI_Video_Generation)"
sys.path.insert(0, str(AGENT2_DIR))

from dotenv import load_dotenv
load_dotenv(dotenv_path=AGENT1_SRC.parent / ".env")
load_dotenv(dotenv_path=AGENT2_DIR / ".env")
...
async def main():
    ...
    from langgraph_browser_agent.browser_pool import shutdown_browser_pool
